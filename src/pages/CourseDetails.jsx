...
const CourseCard = ({ title, fee, desc, duration, careerPath, summary }) => {
    ...
    return (
        <motion.div
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.98 }}
            className="bg-white rounded-xl shadow-lg transition-all duration-300 w-60 md:w-64 lg:w-72 flex flex-col justify-between p-4 border border-purple-100 relative min-h-[280px] overflow-hidden"
        >
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <button
                    className="text-sm text-pink-500 underline hover:text-pink-700 transition mt-2 self-start"
                    onClick={() => setShowDesc(!showDesc)}
                >
                    {showDesc ? 'Back to Summary' : 'See Description'}
                </button>
...
const CourseSection = ({ title, data }) => (
    ...
    >
        <motion.h2
            variants={fadeInUp}
            className="text-3xl md:text-4xl font-extrabold text-purple-800 text-center mb-10 relative inline-block before:absolute before:-bottom-2 before:left-1/2 before:-translate-x-1/2 before:w-16 before:h-1.5 before:bg-pink-400 before:rounded-full"
        >
            {title}
        </motion.h2>
        <div className="flex flex-wrap justify-center gap-6 items-start">
    ...
    return (
        ...
        >
            <motion.h1
                variants={fadeInUp}
                className="text-center text-6xl md:text-7xl font-extrabold text-transparent bg-clip-text 
                bg-gradient-to-r from-purple-500 to-pink-500 mb-7 -mt-4 drop-shadow-md"
            >
                Courses We Offer
            </motion.h1>

            {/* 📊 Infographic with category filter */}
            <CourseInfographic onSelectCategory={handleCategorySelect} />
            ...
            {selectedCategory ? (
                <CourseSection
                    ...
                    title={`${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Courses`}
                    data={courseData[categoryMap[selectedCategory]]}
                />
            ) : (
                <p className="text-center text-purple-700 mt-6 text-xl font-semibold tracking-wide animate-none">
                    🚀 Please select a category to explore our courses!
                </p>
            )}
